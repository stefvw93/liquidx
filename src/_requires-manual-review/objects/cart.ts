...
class Cart extends LiquidObject {
	/**
	 * Additional attributes entered by the customer with the cart.
	 */
	@LiquidObject.property() attributes = new Dictionary(() => {
		throw new Error("Unknown dictionary type for 'attributes'");
	});
	 ...
	 */
	@LiquidObject.property() currency = new Dictionary(() => {
		throw new Error("Unknown dictionary type for 'currency'");
	});
